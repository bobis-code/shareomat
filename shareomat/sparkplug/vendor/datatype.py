# -*- coding: utf-8 -*-
"""
File: emsomat/sparkplug/vendor/datatype.py

Vendored from pysparkplug._datatype (Apache 2.0), see NOTICE.md.
"""
...
def _encode_numeric_array(values: Sequence[float], format_char: str) -> bytes:
    format_str = f"<{len(values)}{format_char}"
    try:
...
def _decode_numeric_array(data: bytes, format_char: str) -> tuple[float, ...]:
    ...
    count = len(data) // size
    format_str = f"<{count}{format_char}"
    try:
