# -*- coding: utf-8 -*-
"""
File: shareomat/web/reports.py

Purpose:
    Report discovery and HTML rendering helpers for the admin dashboard
    and the "Berichte" page. Ported from shareomat/ha/web_reports.py —
    the report table rendering itself stays hand-built HTML (returned as
    a fragment and marked `| safe` in the Jinja2 template) rather than
    being rewritten into a template, since it is dense tabular output
    that was already working and tested.
...
def _reports_path() -> Path | None:
    runtime = get_state().runtime
    return runtime.paths.reports if runtime else None
...
def render_report_section(selected_stem: str, ingress_path: str) -> str:
    ...
    dl_links = " &nbsp;&middot;&nbsp; ".join(
        f'<a href="{ingress_path}/reports/download?f={_html_escape(fn)}" '
